...
function Button({ text, type, width, radius, onClick }: Prop) {
  ...
  return (
    <Btn btnType={btnType} onClick={onClick} btnWidth={btnWidth} btnRadius={btnRadius}>
      {text}
    </Btn>
  );
}
...
const Btn = styled.button<{
  ...
  font-weight: 500;
  width: ${(props) =>
    props.btnWidth === 'middle' ? '140px' : props.btnWidth === 'short' ? '70px' : '290px'};
  border: ${(props) => (props.btnType === 'white' ? '1px solid #D9D9D9' : 'none')};
