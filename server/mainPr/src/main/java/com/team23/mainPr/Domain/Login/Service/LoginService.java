...
public class LoginService {
    ...
    public String doLogin(DoLoginDto dto) {
        ...
        memberRepository.findByLoginId(dto.getLoginId())
            .ifPresentOrElse(findmember -> loginMember[0] = findmember, () -> {
            ...
            login -> memberRepository.findById(login.getMemberId()).ifPresent(member -> {
                ...
                if (member.getPassword().equals(dto.getPassword())) {
                    ...
                    loginRepository.save(
                        Login.builder().lastLoginDate(defaultTimeZone.getNow()).logoutDate(null)
                            .token(token[0]).logouted(false).memberId(member.getMemberId())
                            .build());
                }
                else
                    throw new CustomException(NOT_MATCHED_PASSWORD);
